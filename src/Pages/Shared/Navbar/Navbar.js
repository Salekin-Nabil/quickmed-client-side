...
const Navbar = () => {
    ...
    const menuItems = <React.Fragment>
        ...
        <li>
          {loading ? (
            // Show a loading spinner or placeholder while the user is being loaded
            <div className="flex items-center justify-center">
              <span className="loader"></span> {/* Replace with a spinner component if needed */}
            </div>
          ) : user ? (
            // Display user-related UI once loaded
            <div className="flex p-0">
              <Link
                to="/login"
                className="btn btn-ghost font-bold text-[#cf7b7b] hover:text-white"
                onClick={logout_loader}
              >
                Log Out
              </Link>
              <img
                className="rounded-full w-[40px] h-[40px] ring ring-[#76d276]"
                src={user?.photoURL || "https://graph.facebook.com/1840598326336872/picture"}
                alt=""
              />
            </div> 
          ) : (
            ...
            <Link
              to="/login"
              className="bg-gradient-to-br from-accent to-secondary text-white hover:text-slate-300"
            >
              Login
            </Link>
          )}
    ...
    return (
        <div className="navbar bg-base-100 flex justify-between ">
            <div className="navbar-start">
                ...
                <div><Link to="/"><img src={quickmed} alt="QuickMed" className="btn btn-ghost lg:w-1/4 lg:h-1/4 normal-case text-xl"></img></Link></div>
            </div>
